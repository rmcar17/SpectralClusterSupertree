"""
BL Spectral Cluster Supertree - Spectral Cluster Supertree, but this time with branch lengths
"""
...
def bl_spectral_cluster_supertree(
    ...
    for component in components:
        ...
        child_trees.append(
            bl_spectral_cluster_supertree(new_induced_trees, new_weights)
        )
...
def _proper_cluster_graph_edges(
    ...
    max_weights = (
        {}
    )  # Max weight possible for each vertex TODO: update how max_weights are handled

    for vertex in pcg_vertices:
        edges[vertex] = set()
        max_weights[vertex] = 0  # TODO: update how max weights are handled

    # total = 0

    # TODO, there is a faster way then the current implementation (split one level at a time, over split everything has same weight)
    for tree, weight in zip(trees, weights):
        side: TreeNode
        for side in tree:
            tips: List[TreeNode] = side.tips()
            # start = time.time()
            # print("Len", len(names))
            for i in range(0, len(tips)):
                ...
                for j in range(i):
                    ...
                    edge_weights[edge] = (
                        ...
                        + last_common_ancestor_weight(tips[i], tips[j]) * weight
                    )  # TODO: Normalise weight between 0 and 1
...
def last_common_ancestor_weight(tip_1: TreeNode, tip_2: TreeNode) -> float:
    lca = tip_1.last_common_ancestor(tip_2)
